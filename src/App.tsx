...
// Card Imports for Bento Layout
import CollapsibleCard from './components/Sidebar/CollapsibleCard';
import ComparisonCard from './components/Sidebar/ComparisonCard';
import CommunityCard from './components/Sidebar/CommunityCard';
import ForensicsCard from './components/Sidebar/ForensicsCard';
import MultipliersCard from './components/Sidebar/MultipliersCard';
...
export default function App() {
  ...
  const helpContent: Record<string, { title: string; body: string; impact: string }> = {
    ...
    bot_density: {
      title: "Bot-Dichte",
      body: "Analyse von Account-Metadaten: Alter des Profils, Handle-Struktur (Vokalarmut & Ziffern-Suffixe) sowie Server-Anomalien (PDS-Hosting). Ein hoher Wert deutet auf automatisierte Kampagnen hin.",
    ...
    suspect_score: {
      ...
      body: "Kombinierter Score aus Synchronität, Account-Alter, Handle-Entropie (Zufälligkeit), Server-Typ, Profil-Vollständigkeit und Follow-Ratios. Ein Score > 65 gilt als hochgradig verdächtig.",
    ...
    fingerprinting: {
      ...
      body: "Analyse technischer und administrativer Gemeinsamkeiten. Wir identifizieren 'Digital Fingerprints' durch gemeinsam genutzte externe Domains und 'Batch Creations', also Gruppen von Accounts, die am exakt selben Tag erstellt wurden.",
      impact: "Zentraler Beweis für die Nutzung von Account-Templates und professionellen Bot-Infrastrukturen."
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-cyber-black flex flex-col font-sans text-slate-300">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto flex flex-col">
        {/* Top KPI Row */}
        <StatSummary forensics={forensics} isLoading={isLoading} />

        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Left Column: Context & History */}
          <div className="lg:col-span-3 space-y-6 overflow-y-auto pr-2 custom-scrollbar max-h-[calc(100vh-280px)] pb-10">
            {selectedNode ? (
              <NodeDetailCard 
              ...
              <>
                ...
                {(narrativeSummary || isAnalyzing) && (
                  <CollapsibleCard 
                    title="Dossier Briefing" 
                    icon={<Brain className="w-4 h-4 text-neon-blue" />}
                    subtitle="KI-Lagebericht"
                  >
                    <BriefingBanner 
          ...
          {/* Right Column: Forensics & Impact */}
          <div className="lg:col-span-3 space-y-6 overflow-y-auto pr-2 custom-scrollbar max-h-[calc(100vh-280px)] pb-10">
            <div className="flex items-center gap-3 px-1 mb-2">
              <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">Forensic Lab</span>
              <div className="h-px flex-1 bg-slate-800"></div>
            </div>
