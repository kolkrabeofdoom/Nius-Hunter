...
export default function NetworkView({
  ...
  return (
    <div className={`
      ${isCollapsed ? 'w-16 min-h-0' : 'flex-1 min-h-[500px]'} 
      bg-cyber-dark rounded-3xl border border-slate-800 shadow-2xl relative overflow-hidden flex flex-col transition-all duration-500 ease-in-out
    `}>
      {/* Grid Background */}
      {!isCollapsed && (
      ...
      <div className={`absolute top-6 left-6 z-[50]`}>
        <button 
          ...
          title={isCollapsed ? "Graph ausklappen" : "Graph einklappen"}
        >
          {isCollapsed ? (
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13l-6-6-6 6"/><path d="M18 19l-6-6-6 6"/></svg>
          )}
        </button>
      </div>
      ...
      {isCollapsed && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="rotate-90 text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] whitespace-nowrap">
            NETWORK_VISUALIZER // COLLAPSED
          </div>
        </div>
      )}
      ...
      {!isCollapsed && (
        <>
          {/* Floating Briefing Banner removed and moved to Sidebar */}
